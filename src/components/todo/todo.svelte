<script>
    ...
    const todo = createTodoStore([{ done: false, description: 'write some docs' },
...
<div class="board">
	<input
		...
		on:keydown={(e) => {
			if (e.key === 'Enter') {
				todo.add(e.currentTarget.value);
	...
	<div class="todo">
		<h2>todo</h2>
		<TodoList store={todo} done={false} />
	</div>

	<div class="done">
		<h2>done</h2>
		<TodoList store={todo} done={true} />
	</div>
</div>
